...
(function ($) {
	...
	PivotDataWebPivotBuilder.prototype.setPivotTableConfig = function (cube, pivotConfig) {
		...
		if (!pivotConfig) {
			...
			if (!this.options.allowEmptyReport && newCubeInfo.selectedRows != null
				&& newCubeInfo.selectedRows.length > 0) {
				newCubeInfo.selectedRows.forEach(item => {
					pivotConfig.Rows.push({
						"Name": item.name
					})
				});
			}
			if (!this.options.allowEmptyReport && newCubeInfo.selectedColumns != null
				&& newCubeInfo.selectedColumns.length > 0) {
				newCubeInfo.selectedColumns.forEach(item => {
					pivotConfig.Columns.push({
						"Name": item.name
					})
				});
			}
			if (!this.options.allowEmptyReport && newCubeInfo.selectedMeasures != null
				&& newCubeInfo.selectedMeasures.length > 0) {
				newCubeInfo.selectedMeasures.forEach(item => {
					pivotConfig.Measures.push({
						"Name": item.name
					})
				});
			}
		...
		// clear filter
		let filterVal = pivotConfig.Filter != "" ? pivotConfig.Filter : newCubeInfo.mdxQueryFilter;
		this.$filter.val(filterVal);
